...
sha256_t sha256_hash(uint8_t* data, size_t len) {
    debug("Hashing data of length %d", len);
...
void sha256_append(sha256_ingest_t* ingest, uint8_t* data, size_t len)
{
    if (ingest == nullptr) panic("ingest must not be null");
    if (data == nullptr) panic("data must not be null");
    if (ingest->chunk_count == (uint64_t)-1) panic("ingest must not already be completed!");

    if (ingest->chunk_ptr == 0 && ingest->chunk_count == 0) {
        ingest->result = initial;
    }
    debug("Appending data of length %d", len);
