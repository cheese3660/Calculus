...
struct stream_info
{
    ...
    ssize_t (*write)(struct stream_header *stream, void *buffer, size_t buffer_size);
    void (*flush)(struct stream_header *stream);
    void (*close)(struct stream_header *stream);
    const char *(*error)(struct stream_header *stream);
};
...
stream_t sm_file(FILE *file) __attribute__((nonnull(1)));
...
stream_t sm_memwrite(void **buffer, size_t *buffer_size) __attribute__((access(read_write, 1), access(read_write, 2), nonnull(1, 2)));
...
stream_t sm_memread(void *buffer, size_t buffer_size) __attribute__((access(read_only, 1, 2), nonnull(1)));
...
ssize_t sm_read(stream_t stream, void *buffer, size_t buffer_size) __attribute__((access(write_only, 2, 3), nonnull(1,2)));
...
ssize_t sm_write(stream_t stream, void *buffer, size_t buffer_size) __attribute__((access(read_only, 2, 3), nonnull(1,2)));

/// @brief Flush a stream
/// @param stream The stream to flush
/// @return The status of the stream after the flush
enum stream_status sm_flush(stream_t stream) __attribute__((nonnull(1)));

/// @brief Close a stream
/// @param stream The stream to close, this also free()'s the stream
/// @return The status of the stream after the close
enum stream_status sm_close(stream_t stream) __attribute__((nonnull(1)));
...
enum stream_status sm_w8(stream_t stream, uint8_t u8) __attribute__((nonnull(1)));

/// @brief Write a 16 bit number to the stream
/// @param stream The stream to write the number to
/// @param u16 The 16 bit number
/// @return The new status of the stream after the call
enum stream_status sm_w16(stream_t stream, uint16_t u16) __attribute__((nonnull(1)));

/// @brief Write an 32 bit number to the stream
/// @param stream The stream to write the number to
/// @param u32 The 32 bit number
/// @return The new status of the stream after the call
enum stream_status sm_w32(stream_t stream, uint32_t u32) __attribute__((nonnull(1)));

/// @brief Write an 64 bit number to the stream
/// @param stream The stream to write the number to
/// @param u64 The 64 bit number
/// @return The new status of the stream after the call
enum stream_status sm_w64(stream_t stream, uint64_t u64) __attribute__((nonnull(1)));
...
/// @return The new status of the stream after the call
enum stream_status sm_r8(stream_t stream, uint8_t *u8) __attribute__((access(write_only, 2), nonnull(1,2)));

/// @brief Read a 16 bit number from the stream
/// @param stream The stream to read the number from
/// @param u16 An out parameter for the 16 bit number
/// @return The new status of the stream after the call
enum stream_status sm_r16(stream_t stream, uint16_t *u16) __attribute__((access(write_only, 2), nonnull(1,2)));

/// @brief Read a 32 bit number from the stream
/// @param stream The stream to read the number from
/// @param u32 An out parameter for the 32 bit number
/// @return The new status of the stream after the call
enum stream_status sm_r32(stream_t stream, uint32_t *u32) __attribute__((access(write_only, 2), nonnull(1,2)));

/// @brief Read a 64 bit number from the stream
/// @param stream The stream to read the number from
/// @param u64 An out parameter for the 64 bit number
/// @return The new status of the stream after the call
enum stream_status sm_r64(stream_t stream, uint64_t *u64) __attribute__((access(write_only, 2), nonnull(1,2)));
...
const char *stream_error(stream_t stream) __attribute__((nonnull(1)));

/// @brief Check if a stream is bad (i.e. if it errored or was truncated), closed is not considered bad here.
/// @param status The status code to check
#define STREAM_BAD(status) ({                                \
    typeof_unqual(status) _status = status;                  \
    _status == STREAM_ERRORED || _status == STREAM_TRUNCATED \
})
