...
// Hash a block of data as sha256 - Used for content addressing
// @param data the block of data to hash
// @param len the length of the data being hashed
// @return the sha256 hash of the data
sha256_t sha256_hash(uint8_t* data, size_t len);

#define sha256_string(str) sha256_hash(str,strlen(str))

// Convert a hash to a hex string, the returned data is in a preallocated buffer and will be overwritten on the next call (different than the b64 buffer)
...
// Convert a hash to a base64 string, the returned data is in a preallocated buffer and will be overwritten on the next call (different than the hex buffer)
// @param hash the sha256 hash
...
// Append data to an in progress ingest
...
void sha256_append(sha256_ingest_t* to_ingest, uint8_t* data, size_t len);

// Finalize an in progress ingest and get the digest
